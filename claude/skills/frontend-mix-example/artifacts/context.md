...
### Present in worktree
```
<workspace-root>/.archon/workspaces/<workspace>/worktrees/archon/task-feat-cosmic-explorer/
├── CLAUDE.md              # Parent-workspace documentation (not app-related)
├── docker-compose.yml     # Workspace-level compose (not the app)
├── master.env.example     # All env vars for the parent workspace
├── README.md              # Parent-workspace README
...
**Workspace master.env.example** lists parent-workspace keys (ANTHROPIC_API_KEY, OPENAI_API_KEY, Slack, Asana, etc.) — irrelevant to this app.
