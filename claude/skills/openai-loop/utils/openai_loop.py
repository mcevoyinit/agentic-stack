#!/usr/bin/env python3
"""
OpenAI Loop - Recursive self-deliberation using GPT-5.5 Pro

Runs 3-4 rounds of GPT reasoning with different framings:
...
Usage:
    python3 openai_loop.py --topic "Your question here"
    python3 openai_loop.py --topic "..." --context "background info"
    python3 openai_loop.py --topic "..." --context-file path/to/doc.md --rounds 4
    python3 openai_loop.py --topic "..." --output-dir ./output
"""
...
# Retry config for 429 rate limits / 503 outages
MAX_RETRIES = 8
...
def get_api_key():
    ...
    if _api_key is None:
        ...
        if not _api_key:
            raise ValueError("OPENAI_API_KEY not found in ~/.claude/api-keys.env, .env.local, or environment")
    return _api_key
...
def _is_retryable(error_msg: str, returncode: int) -> bool:
    """Return True if this error is worth retrying (rate limit, 503, timeout)."""
    if returncode in (28, 16, 56):  # curl timeout, HTTP2 framing, network recv error
        return True
    retryable_phrases = [
        "rate_limit", "429", "503", "overloaded", "server_error",
        "temporarily unavailable", "try again later", "capacity",
    ]
    return any(p.lower() in error_msg.lower() for p in retryable_phrases)


def call_openai(prompt: str, system_prompt: str = "", round_label: str = "", verbose: bool = True) -> str:
    """Call OpenAI API with exponential backoff retry. Returns response text or raises."""
    ...
    hdr_path = _auth_hdr_file(f'Authorization: Bearer {api_key}')
    curl_cmd = [
        'curl', '-s', '-S', '--http1.1',
        '-H', f'@{hdr_path}',
    ...
    try:
        ...
        for attempt in range(MAX_RETRIES):
            if verbose:
                ...
                print(f"\n{'='*60}", flush=True)
                print(f"  Calling {OPENAI_MODEL}{label}{attempt_label}...", flush=True)
                print(f"{'='*60}", flush=True)

            t0 = time.time()
            try:
                result = subprocess.run(curl_cmd, input=payload_json, capture_output=True, text=True, timeout=620)
            ...
            if 'choices' not in data or not data['choices']:
                raise RuntimeError(f"No choices in response: {result.stdout[:300]}")
            ...
            if verbose:
                ...
                print(f"  Done in {elapsed:.1f}s | in={usage.get('prompt_tokens','?')} out={usage.get('completion_tokens','?')} tokens", flush=True)
...
2. **Omissions** - What critically important factors were not considered?
...
def run_loop(topic: str, context: str = "", rounds: int = 3, output_dir: str = None) -> dict:
    """
    Run the OpenAI deliberation loop.

    Returns dict with all round outputs.
    """
    print(f"\n{'#'*60}", flush=True)
    print(f"  OPENAI LOOP - {rounds} rounds", flush=True)
    print(f"  Topic: {topic[:80]}{'...' if len(topic) > 80 else ''}", flush=True)
    print(f"{'#'*60}", flush=True)

    system = (
        "You are a world-class analyst and strategic thinker. "
        "You produce rigorous, structured analysis. You do not hedge unnecessarily."
    )
    ...
    r1 = call_openai(r1_analyst_prompt(topic, context), system, "R1 Analyst")
    results["rounds"]["r1_analyst"] = r1
    print(r1, flush=True)

    # Round 2: Adversary
    print(f"\n[R2] ADVERSARY - Attacking Round 1...", flush=True)
    r2 = call_openai(r2_adversary_prompt(topic, r1), system, "R2 Adversary")
    results["rounds"]["r2_adversary"] = r2
    print(r2, flush=True)

    # Round 3: Integrator
    print(f"\n[R3] INTEGRATOR - Synthesizing R1 + R2...", flush=True)
    r3 = call_openai(r3_integrator_prompt(topic, r1, r2), system, "R3 Integrator")
    ...
    if rounds >= 4:
        ...
        r4 = call_openai(r4_decider_prompt(topic, r3), system, "R4 Decider")
    ...
    if output_dir:
        ...
        out_path = Path(output_dir) / f"openai_loop_{ts}"
...
def main():
    parser = argparse.ArgumentParser(
        description="OpenAI Loop - Recursive self-deliberation for maximum insight",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 openai_loop.py --topic "Should we build our own auth vs use Auth0?"
  python3 openai_loop.py --topic "Analyze pivot strategy" --context "We build MCP tools" --rounds 4
  python3 openai_loop.py --topic "..." --context-file context.md --output-dir ./docs/analysis
        """
    )
    ...
    try:
        ...
        print(f"\n\n{'#'*60}", flush=True)
        print("  OPENAI LOOP COMPLETE", flush=True)
        print(f"{'#'*60}\n", flush=True)
