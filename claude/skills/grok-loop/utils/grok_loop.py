#!/usr/bin/env python3
"""
Grok Loop - Recursive self-deliberation using xAI Grok 4.20

Runs 3-4 rounds of Grok reasoning with different framings:
...
Usage:
    python3 grok_loop.py --topic "Your question here"
    python3 grok_loop.py --topic "..." --context "background info"
    python3 grok_loop.py --topic "..." --context-file path/to/doc.md --rounds 4
    python3 grok_loop.py --topic "..." --output-dir ./output
"""
...
def call_grok(prompt: str, system_prompt: str = "", round_label: str = "", verbose: bool = True) -> str:
    """Call xAI Grok API with exponential backoff retry. Returns response text or raises."""
    ...
    try:
        ...
        for attempt in range(MAX_RETRIES):
            if verbose:
                ...
                print(f"\n{'='*60}", flush=True)
                print(f"  Calling Grok 4.20{label}{attempt_label}...", flush=True)
...
def run_loop(topic: str, context: str = "", rounds: int = 3, output_dir: str = None) -> dict:
    """
    Run the Grok deliberation loop.

    Returns dict with all round outputs.
    """
    print(f"\n{'#'*60}", flush=True)
    print(f"  GROK LOOP - {rounds} rounds", flush=True)
    ...
    system = (
        ...
        "Be direct, unconventional, and technically precise."
    )
    ...
    r1 = call_grok(r1_analyst_prompt(topic, context), system, "R1 Analyst")
    results["rounds"]["r1_analyst"] = r1
    print(r1, flush=True)

    # Round 2: Adversary
    print(f"\n[R2] ADVERSARY - Attacking Round 1...", flush=True)
    r2 = call_grok(r2_adversary_prompt(topic, r1), system, "R2 Adversary")
    results["rounds"]["r2_adversary"] = r2
    print(r2, flush=True)

    # Round 3: Integrator
    print(f"\n[R3] INTEGRATOR - Synthesizing R1 + R2...", flush=True)
    r3 = call_grok(r3_integrator_prompt(topic, r1, r2), system, "R3 Integrator")
    ...
    if rounds >= 4:
        ...
        r4 = call_grok(r4_decider_prompt(topic, r3), system, "R4 Decider")
    ...
    if output_dir:
        ...
        out_path = Path(output_dir) / f"grok_loop_{ts}"
...
def main():
    parser = argparse.ArgumentParser(
        description="Grok Loop - Recursive self-deliberation for maximum insight",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 grok_loop.py --topic "Should we build our own auth vs use Auth0?"
  python3 grok_loop.py --topic "Analyze pivot strategy" --context "We build MCP tools" --rounds 4
  python3 grok_loop.py --topic "..." --context-file context.md --output-dir ./docs/analysis
        """
    )
    ...
    try:
        ...
        print(f"\n\n{'#'*60}", flush=True)
        print("  GROK LOOP COMPLETE", flush=True)
        print(f"{'#'*60}\n", flush=True)
